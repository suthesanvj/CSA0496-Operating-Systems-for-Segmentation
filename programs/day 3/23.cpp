...
void firstFit(struct MemoryBlock* memory, int numBlocks, int requestSize) {
    ...
    // Find the first available block that fits the request
    for (int i = 0; i < numBlocks; ++i) {
    ...
    if (firstFitIndex != -1) {
        ...
        printf("Allocated %d units of memory from Block %d using First Fit.\n\n",
               requestSize, firstFitIndex + 1);
    } else {
        printf("Memory allocation failed for request of size %d using First Fit.\n\n",
               requestSize);
    }
}
